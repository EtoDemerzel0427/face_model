...
def fitting_shape(exp3d, pt2d, rot, t3d, f, w_id, single_value):
    """
    ...
    :param w_id: The initial parameters of identity.
    :param single_value:
    :return:
    """

    exp3d = exp3d.T.reshape(-1, pt2d.shape[0], 3).transpose(2, 1, 0) # 3 x 87 x 50
    exp2d = f * np.tensordot(rot, exp3d, axes=(1, 0))[:2, :, :]  # 2 x 87 x 50
    ...
    weight = exp2d.T.dot(exp2d) + beta * regular
